...
<?php 
	...
	if(isset($_POST['action']) && $_POST['action'] == 'resetPass') {
		...
		if(is_array($userData) && count($userData)>0) {
			...
			if($objUser->updateToken()) {
				$url = 'http://' . $_SERVER['SERVER_NAME'] . '' . dirname($_SERVER['REQUEST_URI'],2) . '/user/reset.php?token=' .$urlToken;
	...
	if(isset($_POST['action']) && $_POST['action'] == 'register') {
		...
		if($userData['email'] == $users['uemail']) {
	 	...
	 	if($objUser->save()) {
	 		...
	 		$token = sha1($lastId);
	 		$url = 'http://' . $_SERVER['SERVER_NAME'] . '' . dirname($_SERVER['REQUEST_URI'],2) . '/user/verify.php?id=' . $lastId . '&token=' .$token;
			...
			if(!$mail->send()) {
				...
				echo json_encode( ["status" => 1, "msg" => "Congratulation, Your registration done on our site. Please verify your email."] );
			}
	...
	function validateUpdatePassForm() {
		...
		if(false == $users['pass']) {
			echo json_encode( ["status" => 0, "msg" => "Enter valid pass"] );
			exit;
		}
	...
	function validateRegForm() {
		...
		$users['cfm_pass'] = filter_input(INPUT_POST, 'cfm_pass', FILTER_SANITIZE_STRING);
		if(false == $users['cfm_pass']) {
			echo json_encode( ["status" => 0, "msg" => "Enter valid valid confirm pass."] );
			exit;
		}
